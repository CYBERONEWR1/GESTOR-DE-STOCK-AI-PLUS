    ...
    <div className="space-y-8 animate-in fade-in duration-500">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div className="space-y-1">
          <h1 className="text-3xl font-bold tracking-tight text-primary">Catálogo de Artículos</h1>
          <p className="text-muted-foreground font-medium">Gestiona los productos e insumos disponibles en stock.</p>
        </div>
          ...
          <DialogContent className="sm:max-w-[500px]">
            ...
            <div className="grid gap-4 py-4">
              <div className="grid grid-cols-4 items-center gap-4">
                <Label htmlFor="nombre" className="text-right font-bold">Nombre</Label>
                <Input 
                  id="nombre" 
              ...
              <div className="grid grid-cols-4 items-center gap-4">
                <Label htmlFor="categoria" className="text-right font-bold">Categoría</Label>
                <Input 
                  id="categoria" 
              ...
              <div className="grid grid-cols-4 items-center gap-4">
                <Label htmlFor="unidad" className="text-right font-bold">Unidad</Label>
                <Select 
              ...
              <div className="flex flex-col gap-2">
                <div className="flex items-center justify-between">
                  ...
                  <Button 
                    ...
                    className="h-8 text-accent border-accent/20 hover:bg-accent/10 font-bold"
                  >
                    {isCargandoIA ? <Loader2 className="mr-2 h-3 w-3 animate-spin" /> : <Sparkles className="mr-2 h-3 w-3" />}
                    Sugerir con IA
                  </Button>
                </div>
                <Textarea 
            ...
            <DialogFooter>
              ...
              <Button onClick={manejarGuardarArticulo} className="bg-primary font-bold">Guardar Artículo</Button>
            </DialogFooter>
          </DialogContent>
        </Dialog>
      </div>

      <div className="flex items-center bg-white rounded-lg px-3 py-2 border shadow-sm max-w-md focus-within:ring-2 focus-within:ring-primary/20 transition-all">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {filtrados.map((articulo) => (
          <Card key={articulo.id} className="group hover:shadow-lg transition-all duration-300 border-none bg-white">
            <CardHeader className="pb-3">
              ...
              <CardTitle className="mt-4 text-xl font-bold group-hover:text-primary transition-colors">{articulo.nombre}</CardTitle>
            ...
            <CardContent className="space-y-4">
              ...
              <div className="pt-4 border-t flex items-center justify-between">
                <div className="flex flex-col">
                  <span className="text-xs text-muted-foreground font-bold uppercase">Stock Actual</span>
                  <span className={`text-2xl font-black ${articulo.stockActual < 5 ? 'text-destructive' : 'text-primary'}`}>
                    {articulo.stockActual}
                  </span>
                </div>
                <div className="text-right">
                  <span className="text-xs text-muted-foreground font-bold uppercase">Unidad</span>
                  <div className="text-sm font-bold capitalize">{articulo.unidad}</div>
                </div>
              </div>
              
              {articulo.stockActual < 5 && (
                <div className="bg-red-50 text-red-700 px-3 py-2 rounded-md flex items-center gap-2 text-xs font-bold animate-pulse">
                  <AlertCircle className="h-4 w-4" />
                  Stock Crítico: Necesita reposición
                </div>
              )}
            </CardContent>
          </Card>
        ...
        {filtrados.length === 0 && (
          <div className="col-span-full py-12 text-center bg-white rounded-xl border-2 border-dashed border-muted">
            ...
            <p className="text-muted-foreground">Intenta con otros términos de búsqueda o añade un nuevo artículo.</p>
          </div>
        )}
      </div>
    </div>
